...
def ballPoseCallback(msg, ball_xv_pub):
	...
	if FIRSTFRAME == True:
		...
		ukf_filter.x, ukf_filter.R = ukfInitValue, np.diag([0.005, 0.005, 0.005])
		
		ukf_filter.Q = np.zeros(shape=(6,6))
		ukf_filter.Q[0:2, 0:2] = ukf_filter.Q[2:4, 2:4] = ukf_filter.Q[4:6, 4:6] = Q_discrete_white_noise(2, dt=dt, var=10000) # 三组协方差一致
		return
	else:
		# 更新滤波器
